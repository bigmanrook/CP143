...
struct Person{
    ...
    char personalitytype[100]; //Maybe use pointer for dynamic string size? How to implement this...
...
int main()
{
    int *ptr, x, *dynamicarray, c1, *dynamic2darray, r, c, size;
    struct Person jude = {"INTJ", 2006, 8, 2, "Christian", "Jude", "Wells"};
    struct Person *jw = &jude;
    printf("My name is: %s\n", jw->name); //Access struct in same manner as per normal
